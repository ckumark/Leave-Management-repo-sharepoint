...
> {
  ...
  public RejectRequest(RequestID: number): void {
    PendingApprovalsAPI.updatePendingRequest(
      RequestID,
      LeaveRequestState.Rejected
    )
      .then((response) => {
        if (response === true) {
          this.props.setMessage(
            "Leave Request Rejected Succesfully",
  ...
  public ApproveRequest(RequestID: number): void {
    PendingApprovalsAPI.updatePendingRequest(
      ...
      .then((response) => {
        if (response === true) {
          this.props.setMessage(
            "Leave Request Approved Succesfully",
            InfoState.Success
          );
          this.RetrievePendignApprovals();
        }
                        ...
                        <tr>
                          ...
                          <td>
                            {Util.localDateFormat(
                              pendingRequest.ApplicationDate
                            )}
                            }
                          </td>
                          <td>
                          ...
                          {pendingRequest.Status.toLowerCase() ===
                            ...
                            <td>
                              <button
                                onClick={() =>
                                  this.ApproveRequest(pendingRequest.RequestID)
                                }
                              >
                                Approve
                              </button>
                              <button
                                onClick={() =>
                                  this.RejectRequest(pendingRequest.RequestID)
                                }
                              >
                                {" "}
                                Reject
