...
export default class PendingApprovalsAPI {
  public static async updatePendingRequest(
    RequestID: number,
    UpdatedState: LeaveRequestState
  ): Promise<boolean> {
    try {
      const response = await ListFactory.updateListItem(
        "LeaveBalanceSheet",
        RequestID,
        { Status: UpdatedState }
      );
  ...
  public static async getPendingApprovals(userId: number): Promise<any> {
    try {
      const response = await ListFactory.getListItems(
        "LeaveBalanceSheet",
        ["ID", "UserNameId", "ApproverIdId", "Status", "LeaveDate", "Created"],
        "",
        ""
      );
