...
export default class LibraryFactory{
    ...
    public static async getLibraryItems(context:IWebPartContext,libraryName:string):Promise<Files>{
    ...
    public static async removeLibraryItem(context:IWebPartContext,libraryName:string, fileName:string):Promise<void>{
              
        let web: Web = new Web(context.pageContext.web.absoluteUrl);
        return await web.lists.getByTitle(libraryName).rootFolder.files.getByName(fileName).delete();
    }
}