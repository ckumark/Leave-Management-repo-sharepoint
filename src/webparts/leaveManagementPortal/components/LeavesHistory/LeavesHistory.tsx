...
export default class LeavesHistory extends React.Component<ILeavesHistoryProps, {}>{
    ...
    public GetLeaveRequests(): void {
        LeaveApplicationAPI.getLeaveRequests(this.props.userId).then(requests => {
            this.props.setMessage("", InfoState.Informational); this.leaveRequests = requests;
            if (requests.length > 0) {
                this.leaveRequests.forEach((request) => {
                    request.ApproverEmail = this.props.getApproverEmail(request.ApproverEmail);
                });
                this.forceUpdate();
            }
        }).catch(error => {
    ...
    public cancelLeaveRequest(requestId: number): void {
        ...
        this.props.setMessage("Leave request has been successfully cancelled", InfoState.Informational);
                ...
                <div className="container">
                    <div className="row mt-5 pt-5">
                        <div className="col text-center">
                            ...
                            {(this.leaveRequests === null || this.leaveRequests.length <= 0) && <div>No Leave requests found for the user</div>}
                            {
                                        ...
                                        {this.leaveRequests.map(request => {
                                            return (
                                                <tr>
                                                    ...
                                                    <td>
                                                        {Util.localDateFormat(request.RequestedOn)}
                                                    </td>
                                                    <td>{request.Status}</td>
                                                    {request.Status.toLowerCase() === LeaveRequestState.Pending.toLowerCase() ?
                                                        <td><button onClick={(event) => this.cancelLeaveRequest(request.RequestID)}>Cancel Request</button></td> :
