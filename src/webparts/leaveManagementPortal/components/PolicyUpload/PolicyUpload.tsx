...
export default class PolicyUpload extends React.Component<IPolicyUploadProps, {}>{
    ...
    private filesave() {
        var file = (document.querySelector("#newfile") as HTMLInputElement).files[0];
        if (file != undefined || file != null) {
            // you can adjust this number to control what size files are uploaded in chunks
            if (file.size <= 10485760) {
                // small upload
                LeavePoliciesAPI.uploadLeavePolicies(this.props.context, "Company%20Policies/", file.name, file, true).then(response => {
                    if (response) {
